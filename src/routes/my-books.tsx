...
function MyBooks() {
  ...
  const Section = ({ s }: { s: string }) => {
    const list = by(s);
    if (list.length === 0) return <EmptyState icon={Bookmark} title="Nothing here yet" message="Books you save will show up here." ctaLabel="Browse the library" ctaTo="/library" />;
    return (
